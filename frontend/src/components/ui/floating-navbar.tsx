...
interface NavItem {
  name: string;
  id: string; // Added ID for tab switching logic
  icon: LucideIcon;
}
...
export function FloatingNavBar({ items, activeTab, onTabChange, className }: NavBarProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1 bg-black/40 border border-white/10 backdrop-blur-xl py-2 px-2 rounded-full shadow-2xl shadow-purple-500/10">
        {items.map((item) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <motion.div
                  layoutId="lamp"
                  className="absolute inset-0 w-full bg-white/5 rounded-full -z-10 border border-white/10"
                  initial={false}
                  transition={{
                    type: "spring",
                    stiffness: 300,
